...
from .kds import current_display_orders, current_kds_tickets, kds_group_name, live_orders_group_name
...
class LiveOrdersConsumer(AsyncJsonWebsocketConsumer):
    async def connect(self):
        ...
        self.group_name = live_orders_group_name(self.cafe.id)
        await self.channel_layer.group_add(self.group_name, self.channel_name)
        await self.accept()
        await self.send_json({
            "event": "snapshot",
            "orders": await database_sync_to_async(current_display_orders)(self.cafe),
        })
    ...
    async def live_order(self, event):
        await self.send_json({
            "event": event["event"],
            "order": event["order"],
        })
